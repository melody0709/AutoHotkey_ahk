...
*   📷 **微信 WebP 自动转换：** 在微信窗口粘贴图片时，自动将 WebP 格式转为 JPG，依赖 FFmpeg。
...
在微信窗口内粘贴图片时，自动将剪贴板中的 `.webp` 文件转换为 `.jpg` 格式后粘贴，解决微信粘贴 WebP 图片的兼容性问题。

*   **`Ctrl + V`** (仅微信窗口): 自动检测剪贴板中的 WebP 图片，调用 FFmpeg 转换为 JPG 后粘贴。其他场景（非 WebP 文件、非微信窗口）完全不干预，正常粘贴。
*   **`Alt + F12`**: 开启/关闭 WebP 自动转换功能。

**工作原理：** 通过 `OnClipboardChange` 事件预缓存剪贴板中的 WebP 文件路径，避免在热键钩子内访问剪贴板导致超时误判。转换后的临时文件会在 30 秒后自动清理。
